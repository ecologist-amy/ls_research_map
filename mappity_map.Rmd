...
ba3 <- ba2 %>% 
  add_row(Location = "Global Trends", lat = "108.00000", long = "15", abiotic = 68, biotic = 21, both = 13, total = 102, radius = 22, groups = ">29", )
...
eco2$long <- as.character(eco2$lat)
...
mp2 + geom_scatterpie(aes(x = long, y = lat, r=radius), data = ba3, cols = c("abiotic", "biotic", "both")) +
  ...
  annotate("text", x= 162, y = 120, size = 3, label = "Cross-continent", fontface = "bold") +
  annotate("text", x= 14, y = 138, size = 3, label = "Global Trends", fontface = "bold") +
  annotate("text", x = -130, y = -20, size = 3.5, label = "Number of Articles") +
  annotate("text", x = -155, y = 125, size = 4, label = "A)", fontface = "bold") +
  scale_fill_manual(values = c("#d01c8b", "#4dac26", "#f7f7f7"), name = "Flows", labels = c("Abiotic", "Biotic", "Both")) +
  theme(panel.background = element_blank(), axis.title.x = element_blank(), axis.text.x = element_blank(), axis.title.y = element_blank(), axis.text.y = element_blank(), legend.position = "bottom", legend.box = "horizontal") +
  guides(fill = guide_legend(title.position = "top", title.hjust = 0.5)) +
  geom_scatterpie_legend(ba2$radius, x = -140, y = -50, n = 3, labeller = function(a) a = c("<14", "15-28", ">28")) + 
  annotate("rect", xmin= -23, xmax = 50, ymin = 83, ymax = 145, alpha = 0.2, color = "black", fill = NA)
...
mp2 <- ggplot(world, aes(long, lat)) +
  geom_map(map = world, aes(map_id = region), fill = "gray", color = "white") +
  coord_quickmap()
mp2 + geom_scatterpie(aes(x = long, y = lat, r=radius), data = eco3, cols = c("ter_fre", "fre_mar", "ter_mar", "all")) +
  annotate("text", x= -105, y = 73, size = 3, label = "North America", fontface = "bold") +
  annotate("text", x= 15.5, y = 66, size = 3, label = "Europe", fontface = "bold") +
  annotate("text", x= -57, y = -5, size = 3, label = "South America", fontface = "bold") +
  annotate("text", x= 104, y = 67, size = 3, label = "Asia", fontface = "bold") +
  ...
  annotate("text", x= 14, y = 138, size = 3, label = "Global Trends", fontface = "bold") +
  annotate("text", x = -130, y = -20, size = 3.5, label = "Number of Articles") +
  annotate("text", x = -155, y = 125, size = 4, label = "B)", fontface = "bold") +
  ...
  annotate("rect", xmin=142, xmax = 180, ymin = 85, ymax = 125, alpha = 0.2, color = "black", fill = NA)
...
mp2 + geom_scatterpie(aes(x = long, y = lat, r=radius), data = term2, cols = c("cross-ecosystem", "land_to_sea_connections", "ridge-to-reef")) +
  ...
  geom_scatterpie_legend(term2$radius, x = -140, y = -50, n = 3, labeller = function(a) a = c("<14", "15-28", ">28"))+
   annotate("rect", xmin=142, xmax = 180, ymin = 85, ymax = 130, alpha = 0.2, color = "black", fill = NA)
