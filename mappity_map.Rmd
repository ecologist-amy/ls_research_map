...
htspts <- htspts %>%
  mutate(imp.factor =
           case_when (Source.Title == "ECOLOGICAL MONOGRAPHS" ~ ,
                      Souce.Title == "ECOLOGY LETTERS" ~ ,
                      Source.Title == "PROCEEDINGS OF THE ROYAL SOCIETY B-BIOLOGICAL SCIENCES" ~ , 
                      Source.Title == "AQUATIC MICROBIAL ECOLOGY" ~ ,
                      Source.Title == "HYDROBIOLOGIA" ~ ,
                      Source.Title == "CONSERVATION BIOLOGY ~ ,"))

htspts2 <- htspts %>%
  select(Search_Term, Title, Publication.Year, Location, Linkage, Terrestiral, Freshwater, Marine, Biotic_Abiotic, flows, global_change_y.n, Global_Change, observational, empirical, modelling, review)
...
mp2 + geom_scatterpie(aes(x = long, y = lat, r=radius), data = ba3, cols = c("abiotic", "biotic", "both")) +
  ...
  annotate("text", x= 160, y = 125, size = 3.5, label = "Cross-continent", fontface = "bold") +
  annotate("text", x= 16, y = 150, size = 3.5, label = "Global Trends", fontface = "bold") +
  ...
  theme(panel.background = element_blank(), axis.title.x = element_blank(), axis.text.x = element_blank(), axis.title.y = element_blank(), axis.text.y = element_blank(), axis.ticks.x = element_blank(), axis.ticks.y = element_blank(), legend.position = "bottom", legend.box = "horizontal") +
  guides(fill = guide_legend(title.position = "top", title.hjust = 0.5)) +
  geom_scatterpie_legend(ba3$radius, x = -160, y = -53, n = 3, labeller = function(a) a = c("<14", "15-28", ">28")) + 
  annotate("rect", xmin= -35, xmax = 65, ymin = 94, ymax = 158, alpha = 0.2, color = "black", fill = NA)

ggsave(file = "biotic_abiotic.png", width = 7, height = 4.5, units = "in", dpi = 800)
...
mp3 + geom_scatterpie(aes(x = long, y = lat, r = radius), data = eco3, cols = c("ter_fre", "fre_mar", "ter_mar", "all")) +
  ...
  annotate("text", x= -10, y = -60, size = 3.5, label = "Antarctica", fontface = "bold") +
  annotate("text", x= 161, y = 118, size = 3.5, label = "Cross-continent", fontface = "bold") +
  annotate("text", x= 14, y = 138, size = 3.5, label = "Global Trends", fontface = "bold") +
  annotate("text", x = -140, y = -15, size = 3.5, label = "Number of Articles") +
  annotate("text", x = -170, y = 125, size = 4, label = "B)", fontface = "bold") +
  ...
  annotate("rect", xmin= -31, xmax = 60, ymin = 82, ymax = 146, alpha = 0.2, color = "black", fill = NA)

ggsave(file = "ecosystem_connection.png", width = 7, height = 4.5, unit = "in", dpi = 700)
```
...
flows <- mp2 + geom_scatterpie(aes(x = long, y = lat, r=radius), data = ba3, cols = c("abiotic", "biotic", "both")) +
  annotate("text", x= -103, y = 74, size = 3.5, label = "North America", fontface = "bold") +
  annotate("text", x= 15.5, y = 63, size = 3.5, label = "Europe", fontface = "bold") +
  annotate("text", x= -58, y = -5, size = 3.5, label = "South America", fontface = "bold") +
  annotate("text", x= 105, y = 68, size = 3.5, label = "Asia", fontface = "bold") +
...
ecos <- mp2 + geom_scatterpie(aes(x = long, y = lat, r = radius), data = eco3, cols = c("ter_fre", "fre_mar", "ter_mar", "all")) +
  annotate("text", x= -105, y = 73, size = 3.5, label = "North America", fontface = "bold") +
  annotate("text", x= 15.5, y = 68, size = 3.5, label = "Europe", fontface = "bold") +
  annotate("text", x= -57, y = -2, size = 3.5, label = "South America", fontface = "bold") +
  annotate("text", x= 104, y = 69, size = 3.5, label = "Asia", fontface = "bold") +
  ...
  scale_fill_manual(values = c("#2c7fb8", "#7fcdbb", "#edf8b1"), name = "Flows", labels = c("Abiotic", "Biotic", "Both")) +
  theme(panel.background = element_blank(), axis.title.x = element_blank(), axis.text.x = element_blank(), axis.title.y = element_blank(), axis.text.y = element_blank(), axis.ticks.x = element_blank(), axis.ticks.y = element_blank(), legend.position = "bottom", legend.box = "horizontal") +
  guides(fill = guide_legend(title.position = "top", nrow = 2, title.hjust = 0.5)) +
  scale_fill_manual(values = c("#225ea8", "#41b6c4", "#a1dab4", "#ffffcc"), name = "Ecosystem Connection", labels = c("Terrestrial-Freshwater", "Freshwater-Marine", "Terrestrial-Marine", "All")) +
  geom_scatterpie_legend(eco3$radius, x = -150, y = -50, n = 3, labeller = function(a) a = c("<14", "15-28", ">28"))+
  annotate("rect", xmin= -31, xmax = 60, ymin = 82, ymax = 146, alpha = 0.2, color = "black", fill = NA)

figure <- ggarrange(flows, ecos,
                    labels = c("A", "B"),
                    ncol = 1, nrow = 2)
